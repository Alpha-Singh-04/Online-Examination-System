...
export const UsersTab = ({ isDarkMode, users }) => {
  return (
          ...
          <table className="w-full">
            <thead>
              <tr className={`border-b ${isDarkMode ? "border-white/10" : "border-gray-200"}`}>
                <th className={`text-left py-3 px-4 ${isDarkMode ? "text-white/80" : "text-gray-700"}`}>Name</th>
                <th className={`text-left py-3 px-4 ${isDarkMode ? "text-white/80" : "text-gray-700"}`}>Email</th>
                <th className={`text-left py-3 px-4 ${isDarkMode ? "text-white/80" : "text-gray-700"}`}>Role</th>
            ...
            <tbody>
              {users.length > 0 ? (
                users.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
                          (user.role || index % 3 === 0 ? "admin" : index % 2 === 0 ? "teacher" : "student") === "admin"
                            ? isDarkMode
                              ? "bg-purple-500/20 text-purple-300"
                              : "bg-purple-100 text-purple-800"
                            : (user.role || index % 3 === 0 ? "admin" : index % 2 === 0 ? "teacher" : "student") ===
                                "teacher"
                              ? isDarkMode
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
                          (user.status || index % 5 === 0 ? "inactive" : "active") === "active"
                            ? isDarkMode
                              ? "bg-green-500/20 text-green-300"
                              : "bg-green-100 text-green-800"
                            : isDarkMode
                      ...
                      >
                        {user.status || (index % 5 === 0 ? "Inactive" : "Active")}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                ...
                <tr>
                  <td colSpan={5} className={`py-4 text-center ${isDarkMode ? "text-white/60" : "text-gray-500"}`}>
                    No users found
                  </td>
                </tr>
              )}
